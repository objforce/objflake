...
func init() {
	...
	sf = NewObjflake(st)
	if sf == nil {
		panic("objflake not created")
	}

	startTime = toObjflakeTime(st.StartTime)
...
func TestObjflakeOnce(t *testing.T) {
	sleepTime := uint64(50)
	...
	fmt.Println("objflake id:", id)
	fmt.Println("decompose:", parts)
}

func currentTime() int64 {
	return toObjflakeTime(time.Now())
}

func TestObjflakeFor10Sec(t *testing.T) {
	var numID uint32
	var lastID uint64
...
func TestObjflakeInParallel(t *testing.T) {
...
func TestNilObjflake(t *testing.T) {
	var startInFuture Settings
	startInFuture.StartTime = time.Now().Add(time.Duration(1) * time.Minute)
	if NewObjflake(startInFuture) != nil {
		t.Errorf("objflake starting in the future")
	}
	...
	if NewObjflake(noMachineID) != nil {
		t.Errorf("objflake with no machine id")
	}
	...
	if NewObjflake(invalidMachineID) != nil {
		t.Errorf("objflake with invalid machine id")
	}
}

func pseudoSleep(period time.Duration) {
	sf.startTime -= int64(period) / objflakeTimeUnit
}
